...
import { 
  Calendar, User, Plus, UploadCloud, BrainCircuit, ScanEye, 
  ChevronLeft, ChevronRight, Search, FileImage, AlertCircle 
} from 'lucide-react';

// --- CONFIG ---
const CURRENT_STAFF_ID = "344b6693-0641-4058-a370-29276571f1d5";
const LIMIT = 8; // Số ảnh mỗi trang

interface ImageItem {
  image_id: string;
  file_name: string; // Mới thêm
  original_image_url: string;
  uploaded_by_name: string;
  uploaded_at: string;
  current_status: 'UNLABELED' | 'IN_PROGRESS' | 'SUBMITTED' | 'REJECTED' | 'APPROVED';
...
const ImageGallery = () => {
  ...
  const fetchImages = async () => {
    ...
    try {
      ...
      const response = await getListResultImages(page, LIMIT, "", debouncedSearch);
  ...
  const filteredImages = images.filter((img) => {
    ...
    if (activeTab === 'TODO') return ['UNLABELED', 'IN_PROGRESS', 'REJECTED'].includes(img.current_status);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setIsUploading(true);
      await uploadResultImage(file, CURRENT_STAFF_ID);
  ...
  const renderStatusBadge = (item: ImageItem) => {
    ...
    switch (item.current_status) {
      case "APPROVED":
        ...
        label = "Đã duyệt";
        break;
      case "SUBMITTED": // Thay cho REVIEWED cũ
      ...
      case "REJECTED":
        ...
        label = "Bị từ chối";
        break;
      case "IN_PROGRESS": // Thay cho DRAFT cũ
        colorClass = "bg-orange-100 text-orange-700 border-orange-200";
        label = "Đang làm";
        break;
      case "UNLABELED": // Status cho ảnh Raw
      default:
        colorClass = "bg-gray-100 text-gray-600 border-gray-200";
        label = "Chưa gán nhãn"; // Đây chính là RAW status bạn muốn
        break;
    }

    return (
      <div className="flex flex-col gap-1 items-end">
        <span className={`px-2 py-0.5 rounded text-[10px] font-bold border uppercase ${colorClass}`}>
          {label}
        </span>
        {/* Badge AI riêng biệt */}
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Kho dữ liệu hình ảnh (Raw Data)</h1>
          <p className="text-sm text-gray-500">Quản lý, tìm kiếm và gán nhãn dữ liệu huấn luyện AI</p>
        </div>
        <div>
          ...
          >
            {isUploading ? <UploadCloud className="animate-bounce" size={16} /> : <Plus size={16} />}
            Upload Ảnh Mới
          </button>
        </div>
      </div>
      ...
      {/* --- GRID CONTENT --- */}
      {loading ? (
        <div className="flex justify-center py-20 text-gray-400">Đang tải dữ liệu...</div>
        ...
        <div className="flex flex-col items-center justify-center py-20 text-gray-400 border-2 border-dashed rounded-xl bg-gray-50/50">
          ...
          <p className="text-xs">Thử thay đổi bộ lọc hoặc tìm kiếm từ khóa khác.</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {filteredImages.map((img) => (
            <div
              key={img.image_id}
              onClick={() => router.push(`/labelling/${img.image_id}`)}
              className={`group bg-white rounded-lg border shadow-sm hover:shadow-md cursor-pointer transition-all overflow-hidden flex flex-col h-full ${img.current_status === 'REJECTED' ? 'ring-2 ring-red-200' : ''}`}
            >
              {/* IMAGE THUMBNAIL */}
              ...
              <div className="p-3 text-xs text-gray-500 flex flex-col gap-2 mt-auto bg-white">
                ...
                <div className="flex items-center gap-2" title={img.file_name}>
                   ...
                   <span className="font-semibold text-gray-700 truncate">{img.file_name || 'No Name'}</span>
                </div>
                ...
                <div className="flex justify-between items-center">
                   <div className="flex items-center gap-1.5" title="Người upload">
                      <User size={12} className="text-gray-400"/>
                      <span className="truncate max-w-[80px]">{img.uploaded_by_name}</span>
                   </div>
                   <div className="flex items-center gap-1.5" title="Ngày upload">
                ...
                {['IN_PROGRESS', 'SUBMITTED', 'APPROVED', 'REJECTED'].includes(img.current_status) && img.labeled_by_name && (
                   <div className="mt-1 pt-1 border-t border-gray-100 flex justify-between text-indigo-600">
                      <span>Labeler:</span>
                      <span className="font-semibold truncate max-w-[90px]">{img.labeled_by_name}</span>
                   </div>
                )}
              </div>
            </div>
